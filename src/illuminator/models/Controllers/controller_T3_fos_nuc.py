...
# construct the model
class ControllerT3_fos_nuc(ModelConstructor):
    """
    ...
    parameters={'soc_min': 0,  # Minimum state of charge of the battery before discharging stops
                ...
                'max_fos_gen': 100,     # Maximum power of the generator
                'battery_active': True,
                'elec_assets': True,
    ...
    inputs={'wind_gen': 0,  # Wind power generation
            ...
            'nuc_gen': 0, # Nuclear power generation
    ...
    outputs={'flow2b': 0,  # Power flow to/from battery (positive for charging, negative for discharging)
             'req_pow_fos': 0, # Power flow from fossil generator
             'res_load': 0, # Residue power after wind, solar, nuclear, load
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.req_pow_fos = 0 # Internal state representing the power required from the fossil generator
        ...
        self.max_fos_gen = self.parameters['max_fos_gen']
        self.load_shift = 0
    ...
    def step(self, time: int, inputs: dict=None, max_advance: int=900) -> None:  # step function always needs arguments self, time, inputs and max_advance. Max_advance needs an initial value.
        ...
        Args:
            ...
            - nuc_gen (float): Nuclear power generation [kW]
    ...
    def control(self, wind_gen, pv_gen, nuc_gen, load_dem, soc=None, load_EV=None, load_HP=None, flag_warning = None):
        """
        ...
        if load_EV is not None:
            if load_HP is not None:
                self.res_load = load_dem + load_EV + load_HP - wind_gen - pv_gen  - nuc_gen # kW


            else:
                self.res_load = load_dem + load_EV - wind_gen - pv_gen - nuc_gen


        elif load_HP is not None:
            self.res_load = load_dem + load_HP - wind_gen - pv_gen - nuc_gen

        else:
            self.res_load = load_dem - wind_gen - pv_gen  - nuc_gen # kW
        ...
        if self.battery_active:
            ...
            if self.res_load + self.flow_b > 0:
                self.req_pow_fos = min(self.res_load + self.flow_b, self.max_fos_gen)
                self.dump = -(self.res_load + self.flow_b - self.req_pow_fos)
            else:
                self.req_pow_fos = 0
                self.dump = -(self.res_load + self.flow_b)
        else:
            if self.res_load > 0:
                self.req_pow_fos = min(self.res_load, self.max_fos_gen)
                self.dump = -(self.res_load - self.req_pow_fos)
            else:
                self.req_pow_fos = 0
        ...
        re_params = {'flow2b': self.flow_b, 'req_pow_fos': self.req_pow_fos, 'res_load': self.res_load, 'dump': self.dump}
        # else:
