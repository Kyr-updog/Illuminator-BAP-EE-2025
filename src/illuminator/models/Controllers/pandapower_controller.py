    ...
    def control_and_analyze(self, ncp_powers) -> dict:
        ...
        for element in ncp_powers:
            ...
            if self.peripherals[name]['type'] not in blacklist:
                ...
                elif self.peripherals[name]['type'] == 'PV':
                    total_PV += element[name]
                elif self.peripherals[name]['type'] == 'Wind':
                    total_wind += element[name]
            else:
                ncp_index = pp.get_element_index(self.net, 'load', name)
                self.net.load.at[ncp_index, 'p_mw'] = 0

        residual_demand = total_load - total_PV - total_wind - 500
        ...
        for name, specs in self.peripherals.items():
            if specs['type'] == 'Fossil':
                ...
                total_fos_power += fos_results
                if specs['fos_type'] == 'coal':
                    total_emission += 697.85*1000*fos_results/2
                elif specs['fos_type'] == 'gas':
                    total_emission += 421*1000*fos_results/2
            elif specs['type'] == 'GridConnection':
        ...
        for line_id, connection in self.ss_connections.items():
            ...
            if line['tf'].iloc[0] == 0:
                ...
                congestion1 = abs(line_results['p_from_mw']) / (self.net.line.at[line_index, 'max_i_ka'] * line['prim_kv_rating'])
                congestion2 = abs(line_results['i_ka']) / self.net.line.at[line_index, 'max_i_ka']
                congestion3 = abs(line_results['loading_percent'])
            else:
                ...
                if self.transformers_from_stations[line_id] == 'high':
                    ...
                    congestion1 = abs(trafo_results['p_hv_mw']) / self.net.trafo.at[trafo_index, 'sn_mva']
                    congestion2 = abs(trafo_results['loading_percent'])
                    congestion3 = abs(trafo_results['loading_percent'])
                else:
                    tl_powers.setdefault(connection[0], {})[line_id] = trafo_results['p_lv_mw']
                    congestion1 = abs(trafo_results['p_lv_mw']) / self.net.trafo.at[trafo_index, 'sn_mva']
                    congestion2 = abs(trafo_results['loading_percent'])
                    congestion3 = abs(trafo_results['loading_percent'])
            if line_id == 'line_32':
                one_l_congestion1 = congestion1
                one_l_congestion2 = congestion2
                one_l_congestion3 = congestion3
            elif line_id == 'line_44':
                one_t_congestion1 = congestion1
                one_t_congestion2 = congestion2
                one_t_congestion3 = congestion3
            if congestion3 > int_max_congested[1]:
                int_max_congested = [line_id, congestion3]
        ...
        degrees = self.net.res_bus[['va_degree']].copy()
        degrees = degrees.transpose()
        degrees.to_csv('degrees.csv', mode='a', index=False, header=False)
        ...
        re_params = {'cp_powers': cp_powers, 'tl_powers': tl_powers} # Also battery SoC
